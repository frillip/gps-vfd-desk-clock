...
typedef union
{
  struct __attribute__ ((packed)) _user_prefs_data_struct
  {
    ...
    struct __attribute__((packed))
    {
        struct __attribute__((packed))
        {
            bool enabled : 1; // esp-telnet-enable // esp-telnet-disable // TELNET_ENABLED_DEFAULT 1
            uint16_t padding : 15;
        } flags;
        uint16_t port; // esp-telnet-port // TELNET_PORT_DEFAULT 23
    } telnet;
    struct __attribute__((packed))
    {
		  char server[PREFERENCES_STORED_STRING_SIZE]; // #define CLOCK_NTP_SERVER_DEFAULT "rubidium.darksky.io" // esp-ntp-set-interval
      uint32_t interval; // #define CLOCK_NTP_INTERVAL_DEFAULT 1800 // esp-ntp-set-server
    } ntp;
    struct __attribute__((packed))
    {
        struct __attribute__((packed))
        {
            bool enabled : 1;  // #define REMOTE_TZINFO_ENABLED_DEFAULT 1 // esp-tzinfo-enable
            uint16_t padding : 15;
        } flags;
        ...
        uint32_t interval; // #define REMOTE_TZINFO_INTERVAL_DEFAULT 900 // esp-tzinfo-set-interval
        int32_t gnss_accuracy; // #define REMOTE_TZINFO_GNSS_ACCURACY_DEFAULT 1 // esp-tzinfo-set-acc
    } rtzinfo;
    struct __attribute__((packed))
    {
        struct __attribute__((packed))
        {
            uint16_t auto_enabled : 1; // #define UPDATE_AUTO_ENABLED_DEFAULT 0 // esp-update-auto-enable
            uint16_t auto_time : 8; // #define UPDATE_AUTO_CHECK_LOCAL_TIME_DEFAULT 4 // esp-update-auto-time
            uint16_t padding : 7;
        } flags;
